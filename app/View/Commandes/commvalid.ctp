		...
		<div class="card">
			...
			<table class="table" id='table_das'>
				...
				<?php foreach ($commandes as $commande): ?>
					<tr class="barre" data-toggle="modal" data-target=".bd-example-modal-lg">
						<td  id="<?= $commande['Commande']['id']; ?>" onClick="recupereId(this.id)"><?php echo h($commande['Commande']['id']); ?> &nbsp;</td>
						<td>
							<?php echo $this->Html->link($commande['User']['id'], array('controller' => 'users', 'action' => 'view', $commande['User']['id'])); ?>
						</td>
						<td><?php echo h($commande['Commande']['name']); ?>&nbsp;</td>
						<td><?php echo h($commande['Commande']['tel']); ?>&nbsp;</td>
						<td><?php echo h($commande['Commande']['email']); ?>&nbsp;</td>
						<td><?php echo h($commande['Commande']['adress']); ?>&nbsp;</td>
						<td><?php if($commande['Commande']['state']==0){ ?>
	...
	<div class="modal-dialog modal-lg">
		<div class="modal-content">
			<div class="modal-header">
				<h5 class="modal-title" id="exampleModalLabel">Detals Commande :</h5>
	...
	function recupereId(id_cmnd)
	{
		$.ajax({
			...
			success:function name(data) {
				...
				detail.forEach(ele => {
					$('.modal-footer').empty();
					$('.table_d').append('<tr><th scope="row">'+i+'</th><td>'+ele.produit_id+'</td><td>'+ele.qte+'</td><td>'+ele.prixtot+'</td></tr>');
